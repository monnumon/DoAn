...
export default function CommentKhachhang(props) {
    ...
    async function GuiNhanXet() {
        let res = await axios.post('/comments-them', {
            soSao: comment.soSao,
        ...
        if (res.data.status === 'success') {
            message.success('Nhận xét của bạn đã được đăng');
    ...
    function KiemTraDuLieu() {
        if (comment.soSao === '') {
            ...
            if (comment.noiDung.length < 5) {
                message.error('Nhận xét ít nhất 5 kí tự');
            } else {
                GuiNhanXet();
            }
        }
    }
    console.log(GuiNhanXet);
  
    async function LayDataSoSao() {
        let res = await axios.get('/comments-product?id=' + dataProduct.idShow);
    ...
    async function LayDataCommentTheoOption() {
        let res = await axios.get('/comments-nguoidung?id=' + dataProduct.idShow + '&option=' + nhanXetOption);
    ...
    console.log(LayDataCommentTheoOption);
        ...
        <div className="row" style={{ marginTop: 40 }}>
            <div className="col">
                ...
                <div className="col-sm-9">
                                    ...
                                    Nhận xét về sản phẩm<br></br>
                                        <Button variant='warning' style={{ width: '100%' }} onClick={() => {
                                            if (cookies.userID === undefined ) {
                                                message.error('Vui lòng đăng nhập để gửi nhận xét');
                                            } else {    
                                                if (isAdmin) {
                    ...
                    <div className='row'>
                        <div className='col-sm-3'>
                            <span style={{ fontSize: 16 }}>Chọn xem bình luận</span>
                        </div>
